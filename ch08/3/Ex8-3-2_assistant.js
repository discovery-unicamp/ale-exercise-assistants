...
class Ch8E3_2 extends Assistant_Script{
  constructor(){
    super();
    this.ui = new UI_Helper("Exercise 8.3.2: Stack and Frame Pointers");
    
    let n = this.randint(3, 7);
    let res = this.fibonacci(n);
    ...
    let test = async _ => {
        ...
        let log_msgs = [
          ...
          `Fibonacci result was incorrect.\nReturned ${a0} but should be ${res}.`,
    ...
    this.predefined_args = ["--interactive", "--isa", "acdfimsu"];
    this.ui.add_test("Test fibonacci", test);
  ...
  calls(n){
    if (n > 1){
      return 1 + this.calls(n-1) + this.calls(n-2);
    }
    return 1;
  }
  ...
  get_blob(n, result, max_calls){
    let code = `# Testing Code for Exercise 8.3.2
    .globl _start
